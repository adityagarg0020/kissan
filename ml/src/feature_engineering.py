import numpy as np
import pandas as pd
from preprocessing import add_calendar_features, clean_outliers_iqr

def create_time_series_features(df, price_col='Mandi Modal Price (AgMarknet)'):
    """
    ...
    """
    df = df.copy()
    df = clean_outliers_iqr(df, price_col=price_col)
    ...
    rolling_3 = grouped[price_col].apply(lambda x: x.shift(1).rolling(window=3, min_periods=1).mean())
    df['rolling_mean_3'] = rolling_3.reset_index(level=[0, 1], drop=True)
    
    rolling_std_3 = grouped[price_col].apply(lambda x: x.shift(1).rolling(window=3, min_periods=1).std().fillna(0))
    df['rolling_std_3'] = rolling_std_3.reset_index(level=[0, 1], drop=True)
    
    # 6-month rolling mean
    rolling_6 = grouped[price_col].apply(lambda x: x.shift(1).rolling(window=6, min_periods=2).mean())
    df['rolling_mean_6'] = rolling_6.reset_index(level=[0, 1], drop=True)
    
    # 12-month rolling mean & std
    rolling_12 = grouped[price_col].apply(lambda x: x.shift(1).rolling(window=12, min_periods=3).mean())
    df['rolling_mean_12'] = rolling_12.reset_index(level=[0, 1], drop=True)
